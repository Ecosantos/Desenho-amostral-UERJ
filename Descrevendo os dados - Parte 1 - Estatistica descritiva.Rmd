...
Bom, sabemos que a formula para calcular o Erro padrão da média (ou *Stardard error*, em inglês) do nosso objeto ``x`` (![equation](https://latex.codecogs.com/gif.latex?$SE_{\bar{x}}$)) é dividir o desvio padrão, obtido com a função ``sd()`` e representado pela letra sigma (![equation](https://latex.codecogs.com/gif.latex?\sigma)) pela raiz quadrada do tamanho amostral ``n``, ou seja: (![equation](https://latex.codecogs.com/gif.latex?\rfrac{\sigma}{\sqrt{n}})). Logo, podemos juntar todas essas informações em uma única equação e teremos:
